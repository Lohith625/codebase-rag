...
class DocumentLoader:
    ...
    def __init__(self):
        ...
        logger.info(f"DocumentLoader initialized with {len(self.supported_extensions)} supported file types")
    ...
    def filter_by_extension(
        ...
        logger.info(f"Filtered {len(file_paths)} files to {len(filtered)} with extensions {extensions}")
        return filtered
    ...
    def filter_by_size(
        ...
        filtered = [
            path for path in file_paths
            if min_size <= path.stat().st_size <= max_size
        ]
        logger.info(f"Filtered {len(file_paths)} files to {len(filtered)} by size ({min_size}-{max_size} bytes)")
        return filtered
