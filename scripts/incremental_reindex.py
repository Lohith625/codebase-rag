    ...
    for repo_dir in repos_path.iterdir():
        if repo_dir.is_dir() and (repo_dir / ".git").exists():
            ...
            if all_chunks:
                ...
                logger.info(f"✅ Re-indexed {len(documents)} files ({indexed} chunks) from {repo_dir.name}")
    
    # Save updated index
    if total_chunks > 0:
        indexer.save_index(index_path)
        logger.info(f"✅ Incremental re-indexing complete: {total_files} files, {total_chunks} chunks")
    else:
