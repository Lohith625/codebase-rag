...
'''
    ...
    for func in functions:
        logger.info(f"  - {func['name']} (lines {func['start_line']}-{func['end_line']})")
...
def test_chunker(code: str):
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        logger.info(f"  Lines: {chunk.metadata['start_line']}-{chunk.metadata['end_line']}")
...
def test_with_real_file():
    ...
    test_file = Path(__file__).parent.parent / "backend" / "ingestion" / "github_loader.py"
    ...
    for chunk in chunks[:5]:
        logger.info(f"  - {chunk.metadata['type']}: {chunk.metadata.get('name', 'N/A')} ({chunk.metadata['num_lines']} lines)")
